...
func (m *ConcreateMediator) Write(p []byte) (int, error) {
	m.Received += string(p)
	return len(p), nil
}
...
func (b *Bill) Respond() {
	...
	fmt.Fprintf(b.mediator, "Bill: What?\n")
...
func (t *Ted) Talk() {
	...
	fmt.Fprintf(t.mediator, "Ted: Bill?\n")
	t.mediator.Communicate("Ted")
}

// Respond responds.
func (t *Ted) Respond() {
	fmt.Fprintf(t.mediator, "Ted: Strange things are afoot at the Circle K.\n")
}
